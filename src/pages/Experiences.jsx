...
function seedIfEmpty(){
  ...
  if(!ex){
    const sample = [
      { id: '1', title: 'Frontend Developer', org: 'Acme Co', category: 'Full-time', startDate: '2022-01-01', endDate: '2023-01-01', tags: ['react','javascript'] },
      { id: '2', title: 'Data Analyst', org: 'DataCorp', category: 'Contract', startDate: '2021-05-01', endDate: '2021-12-31', tags: ['sql','python'] },
      { id: '3', title: 'Intern', org: 'Startup', category: 'Internship', startDate: '2020-06-01', endDate: '2020-08-31', tags: ['excel'] }
    ]
...
export default function Experiences(){
  ...
  useEffect(()=>{ writeJSON(LS_KEYS.EXPERIENCES, items) },[items])
  ...
  function askDelete(item){ setConfirm(item) }
  function doDelete(){
    if(!confirm) return
  ...
  async function handleImportFile(event){
    ...
    try {
      ...
      appendAudit('IMPORT', `Imported ${result.experiencesImported} experiences from file ${file.name}`)
    } catch (err) {
      notify({
        type: 'error',
  ...
  function toggleSelectAll(allVisibleIds) {
    if (selectedIds.size === allVisibleIds.length && allVisibleIds.every(id => selectedIds.has(id))) {
  ...
  function handleDeleteSelected() {
    ...
    const confirmed = window.confirm(`Delete ${count} selected experiences? This cannot be undone.`)
    if (!confirmed) return
    ...
    // Delete from items
    setItems(prevItems =>
      prevItems.filter(exp => !selectedIds.has(exp.id))
    )
  ...
  const results = useMemo(()=>{
    ...
    let out = [...items]
    if(ql) out = out.filter(x=> (x.title||'').toLowerCase().includes(ql) || (x.org||'').toLowerCase().includes(ql) || (x.tags||[]).join(' ').toLowerCase().includes(ql))
    ...
    <div className="app">
      ...
      <div className="card">
        ...
        <div className="controls">
          <input className="search-input" placeholder="Search title, org, tags" value={q} onChange={e=>setQ(e.target.value)} />
          <select className="filter-select" value={categoryFilter} onChange={e=>setCategoryFilter(e.target.value)}>
            <option value="">All categories</option>
            {categories.map(c=> <option key={c} value={c}>{c}</option>)}
          </select>
          {admin && (
            <button className="btn" onClick={()=>{ setEditing(null); setModalOpen(true) }}>+ New Experience</button>
          )}
          <button className="btn secondary" onClick={handleExport} title="Download as JSON">↓ Export</button>
          {admin && (
            <>
              <button className="btn secondary" onClick={handleImportClick} title="Import JSON file">↑ Import</button>
              <input
                ref={fileInputRef}
                type="file"
                accept=".json"
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th>Title</th><th>Organization</th><th>Category</th><th>Dates</th><th>Tags</th>{admin && <th></th>}
            </tr>
          </thead>
          <tbody>
            {results.map(it=> (
              <tr key={it.id}>
                ...
                <td>{it.title}</td>
                <td>{it.org}</td>
                <td><span className={`badge ${getCategoryBadgeClass(it.category)}`}>{it.category}</span></td>
                <td>{it.startDate} — {it.endDate}</td>
                <td>{(it.tags||[]).map(t=> <span key={t} className="chip">{t}</span>)}</td>
                {admin && (
                  <td>
                    <button className="btn secondary small" onClick={()=>{ setEditing(it); setModalOpen(true) }}>Edit</button>
                    <button className="btn small danger" onClick={()=>askDelete(it)}>Delete</button>
                  </td>
                )}
              </tr>
            ))}
      ...
      {modalOpen && <ExperienceFormModal initial={editing} onClose={()=>{setModalOpen(false);setEditing(null)}} onSave={saveExperience} />}
      {confirm && <ConfirmDialog message={`Delete ${confirm.title}?`} onCancel={()=>setConfirm(null)} onConfirm={doDelete} />}
    </div>
  )
}
