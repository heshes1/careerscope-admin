            ...
            <ol style={{paddingLeft:20,marginTop:12}}>
              {tagCounts.slice(0,10).map(([tag,count],i)=> (
                <li key={tag} style={{marginBottom:10,color:'#0F172A'}}>
                  <strong>{tag}</strong> <span style={{color:'#475569',fontSize:'13px'}}>({count})</span>
                </li>
              ))}
            </ol>
          ...
          <div style={{flex:1}}>
            ...
            <ul style={{listStyle:'none',paddingLeft:0,marginTop:12}}>
              {categoryTotals.map(([cat,count])=> (
                <li key={cat} style={{marginBottom:12,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
